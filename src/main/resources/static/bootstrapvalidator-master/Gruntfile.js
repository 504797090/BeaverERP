    ...
    grunt.initConfig({
        ...
        copy: {
            main: {
                files: [
                    ...
                    { cwd: '<%= dirs.src %>/jquery/language', src: '**', dest: '<%= dirs.dist %>/jquery/language', expand: true, flatten: true, filter: 'isFile' }
                ]
            }
        },
        ...
        concat: {
            source: {
                ...
                src: ['<%= dirs.src %>/jquery/bootstrapValidator.js', '<%= dirs.src %>/jquery/validator/*.js'],
                dest: '<%= dirs.dist %>/jquery/bootstrapValidator.js'
            },
            test: {
        ...
        uglify: {
            ...
            build: {
                src: ['<%= dirs.dist %>/jquery/bootstrapValidator.js'],
                dest: '<%= dirs.dist %>/jquery/bootstrapValidator.min.js'
            }
        },
        ...
        watch: {
            source: {
                files: ['<%= dirs.src %>/css/**', '<%= dirs.src %>/jquery/**'],
                tasks: ['build'],
                options: {
                    spawn: false
                }
